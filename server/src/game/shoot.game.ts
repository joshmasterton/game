...
export const initializeShoot = (
  socket: Socket,
  engine: Matter.Engine,
  players: Map<string, { body: Matter.Body; health: number }>,
  bullets: Map<number, Matter.Body>
) => {
  ...
  socket.on("shoot", ({ shooterId, targetId }) => {
    ...
    const angle = Math.atan2(
      target.body.position.y - shooter.body.position.y,
      target.body.position.x - shooter.body.position.x
    );
    ...
    const bullet = Matter.Bodies.circle(
      shooter.body.position.x + Math.cos(angle) * 20,
      shooter.body.position.y + Math.sin(angle) * 20,
      5,
      {
        restitution: 1,
        frictionAir: 0.01,
